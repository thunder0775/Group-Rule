...
ALLOWED = {
    ...
    "USER-AGENT", "URL-REGEX", "PROCESS-NAME", "DEST-PORT", "DST-PORT", "GEOIP",
}
...
def parse(text: str, mode: str = "rules", expected_policy: str | None = None):
    rules, seen = [], set()
...
def source_line_stats(text: str, mode: str):
...
def fetch(url: str, timeout: int, max_bytes: int):
    req = urllib.request.Request(url, headers={"User-Agent": "Group-Rule/6.0"})
...
def write_list(path, rules, kind):
    ...
    header = [
        ...
        f"# Type: {kind}",
        f"# Generated: {now()}",
        "# Policy intentionally omitted; assign policy in Shadowrocket .conf.",
    ]
            ...
            for rule in rules:
                ...
                for parent in parents:
                    ...
                    for pcat, pname, prule in suffixes.get(parent, set()):
                        reason = "exact_domain_covered" if kind == "DOMAIN" else "child_suffix_covered"
                        findings.add((rule, prule, category, name, pcat, pname, reason))
                    if parent in suffixes:
                        break
    sample = [
        {"type": item[6], "covered": {"category": item[2], "item": item[3], "rule": item[0]}, "covering": {"category": item[4], "item": item[5], "rule": item[1]}}
        for item in sorted(findings)[:limit]
    ]
...
def semantic_cidr_audit(categories, limit):
    ...
    for version in (4, 6):
        ...
        ordered = sorted((x for x in networks if x[0] == version), key=lambda x: (x[1], x[2], x[3], x[4]))
        for _, prefix_len, address, category, name, rule in ordered:
            node = root
            covered = set()
    ...
    sample = [
        {"rule": rule, "category": category, "item": name, "reason": "covered_by_parent_cidr", "covered_by": {"category": owner_category, "item": owner_name}}
        for rule, category, name, owner_category, owner_name in sorted(findings)[:limit]
    ]
...
def build_outputs(categories, order):
    ...
    outputs = {
        category: list(dict.fromkeys(rule for rules in items.values() for rule in rules if winners.get(rule) == category))
        for category, items in categories.items()
    }
...
def severity_findings(scfg, categories, statuses, cfg_findings, output_audit, domain_audit, cidr_audit):
    findings = list(cfg_findings)
    for key, status in statuses.items():
        if status["state"] == "unavailable":
            findings.append({"severity": "BLOCK", "category": key, "reason": "atomic_rule_unavailable_no_last_known_good"})
        elif status["state"] == "kept_previous":
            findings.append({"severity": "WARNING", "category": key, "reason": "using_last_known_good"})
        for source in status["sources"]:
            if source.get("status") != "ok":
                findings.append({"severity": "WARNING", "category": key, "reason": source.get("reason", source.get("error", "source_error")), "url": source.get("url")})
            elif source.get("redirect_host_changed"):
                findings.append({"severity": "WARNING", "category": key, "reason": "redirect_host_changed", "url": source.get("url"), "final_url": source.get("final_url")})
    if not output_audit["valid"]:
        findings.append({"severity": "BLOCK", "reason": "compiled_output_invalid", "count": output_audit["problem_count"]})
    if domain_audit.get("risky_keyword_count", 0):
        findings.append({"severity": "WARNING", "reason": "risky_domain_keyword", "count": domain_audit["risky_keyword_count"]})
    if domain_audit.get("count", 0):
        findings.append({"severity": "INFO", "reason": "semantic_domain_redundancy", "count": domain_audit["count"]})
    if cidr_audit.get("count", 0):
        findings.append({"severity": "INFO", "reason": "semantic_cidr_redundancy", "count": cidr_audit["count"]})
    return findings
...
def main() -> int:
    ...
    min_default = int(health.get("min_rules", 1))
    min_ratio = float(health.get("min_rule_ratio", 0.2))
    max_ratio = float(health.get("max_rule_ratio", 10.0))
    max_invalid = float(health.get("max_invalid_ratio", 0.8))
    sample_limit = min(int(health.get("max_audit_samples", 200)), 1000)
    ...
    for category, items in scfg.get("sources", {}).items():
        for name, sources in items.items():
            target = atomic_path(category, name)
            ...
            for raw in sources:
                ...
                url = source.get("url", "")
                mode = source.get("mode", "rules")
                policy = source.get("policy")
                min_rules = int(source.get("min_rules", min_default))
                source_min_ratio = float(source.get("min_rule_ratio", min_ratio))
                source_max_ratio = float(source.get("max_rule_ratio", max_ratio))
                source_invalid_ratio = float(source.get("max_invalid_ratio", max_invalid))
                previous_count = previous_source_counts.get(url, 0)
                try:
                    text, meta = fetch(url, timeout, max_bytes)
                    parsed = parse(text, mode, policy)
                    reason, considered, malformed = validate_source(text, parsed, min_rules, previous_count, source_min_ratio, source_max_ratio, source_invalid_ratio, mode)
                    final_host = urllib.parse.urlparse(meta["final_url"]).netloc
                    source_host = urllib.parse.urlparse(url).netloc
                    redirect_changed = bool(final_host and source_host and final_host != source_host)
                    item = {
                        "url": url, "mode": mode, "policy_filter": policy, "min_rules": min_rules,
                        "previous_source_rules": previous_count, "considered_lines": considered, "malformed_lines": malformed,
                        "invalid_ratio": round(malformed / considered, 4) if considered else 0,
                        **meta, "rules": len(parsed), "status": "ok" if reason is None else "rejected",
                        "redirect_host_changed": redirect_changed,
                    }
                    if reason:
                        item["reason"] = reason
                    if reason is None:
                        ...
                        old = history.get(url, {})
                        history[url] = {
                            "sha256": meta["sha256"], "rules": len(parsed),
                            "first_seen": old.get("first_seen", generated), "last_seen": generated,
                            "last_changed": generated if old.get("sha256") != meta["sha256"] else old.get("last_changed", generated),
                            "unchanged_runs": (int(old.get("unchanged_runs", 0)) + 1) if old.get("sha256") == meta["sha256"] else 0,
                        }
                ...
                results.append(item)
            custom = []
            for value in scfg.get("custom", {}).get(f"{category}/{name}.list", []):
                rule = norm(value) if isinstance(value, str) else None
                if rule:
                    custom.append(rule)
            ...
            if candidates and (healthy > 0 or not sources):
                ...
                current, state = previous, "kept_previous"
            else:
                current, state = [], "unavailable"
            categories[category][name] = current
            statuses[f"{category}/{name}"] = {
                "state": state, "rule_count": len(current), "previous_count": len(previous),
                "healthy_sources": healthy, "sources": results,
            }
            quality[f"{category}/{name}"] = validate_rule_set(current)

    order = pcfg.get("category_order", list(categories))
    outputs, winners = build_outputs(categories, order)
    compiled = {}
    for category, rules in outputs.items():
        write_list(RULES / "compiled" / f"{category}.list", rules, f"compiled:{category}")
        compiled[category] = len(rules)
    ...
    domain_audit = semantic_domain_audit(categories, sample_limit)
    cidr_audit = semantic_cidr_audit(categories, sample_limit)
    output_audit = validate_outputs(outputs)
    cfg_findings = config_audit(scfg, pcfg)

    invalid_domain = sum(len(value["invalid_domains"]) for value in quality.values())
    invalid_cidr = sum(len(value["invalid_cidrs"]) for value in quality.values())
    risky_keywords = domain_audit.get("risky_keyword_count", 0) + sum(len(value["risky_keywords"]) for value in quality.values())
    exact_duplicate_occurrences = sum(max(0, len(locs) - 1) for locs in locations.values())

    gate_findings = severity_findings(scfg, categories, statuses, cfg_findings, output_audit, domain_audit, cidr_audit)
    if invalid_domain:
        gate_findings.append({"severity": "BLOCK", "reason": "invalid_domain_in_current_rules", "count": invalid_domain})
    if invalid_cidr:
        gate_findings.append({"severity": "BLOCK", "reason": "invalid_cidr_in_current_rules", "count": invalid_cidr})
    ...
    report = {
        ...
        "conflict_resolution": {rule: {"winner": winners[rule], "categories": sorted(categories)} for rule, categories in conflicts.items()},
        "priority": pcfg,
        ...
        "gate": {"status": "BLOCKED" if blocked else "PASS", "severity_counts": dict(severity_counts), "findings": gate_findings[:sample_limit]},
        "global_quality": {
            "exact_duplicate_occurrences": exact_duplicate_occurrences,
            "same_category_duplicate_rule_count": len(same_category_duplicates),
            "cross_category_conflict_count": len(conflicts),
            "semantic_domain_redundancy_count": domain_audit.get("count", 0),
            "semantic_cidr_redundancy_count": cidr_audit.get("count", 0),
            "invalid_domain_count": invalid_domain,
            "invalid_cidr_count": invalid_cidr,
            "risky_keyword_count": risky_keywords,
            "compiled_output_problem_count": output_audit["problem_count"],
        },
    }
    ...
    md = [
        "# Group-Rule 审计报告", "", f"生成时间：`{generated}`",
        f"发布闸门：**{'BLOCKED' if blocked else 'PASS'}**", "",
        "## 审计等级", "",
        "- `BLOCK`：禁止本次生成结果进入 Git 提交。", "- `ERROR`：严重运行异常（默认不自动发布）。", "- `WARNING`：记录并继续，异常原子规则使用 Last Known Good。", "- `INFO`：信息类质量提示。", "",
        "## 总体质量", "",
        f"- 精确重复出现次数：`{exact_duplicate_occurrences}`",
        f"- 同分类重复规则：`{len(same_category_duplicates)}`",
        f"- 跨分类重复规则：`{len(conflicts)}`",
        f"- DOMAIN 语义冗余：`{domain_audit.get('count', 0)}`",
        f"- CIDR 语义冗余：`{cidr_audit.get('count', 0)}`",
        f"- 无效 DOMAIN：`{invalid_domain}`",
        f"- 无效 CIDR：`{invalid_cidr}`",
        f"- 高风险 DOMAIN-KEYWORD：`{risky_keywords}`",
        "", "## 分类统计", "",
    ]
    md += [f"- `{category}`：{count} 条" for category, count in compiled.items()]
    md += ["", "## 闸门结果", ""]
    md += [f"- `{key}`：`{value}`" for key, value in sorted(severity_counts.items())]
    if gate_findings:
        md += ["", "### Findings", ""] + [f"- **{item['severity']}** `{item.get('reason', '')}`" + (f" — `{item.get('category')}`" if item.get('category') else "") for item in gate_findings[:100]]
    md += ["", "## 语义冗余", "", f"DOMAIN 父子覆盖：`{domain_audit.get('count', 0)}`（排除裸 TLD）", f"CIDR 父网覆盖子网：`{cidr_audit.get('count', 0)}`"]
    if domain_audit.get("samples"):
        md += ["", "### DOMAIN 示例", ""] + [f"- `{item['covered']['rule']}` ← `{item['covering']['rule']}`" for item in domain_audit["samples"][:50]]
    if cidr_audit.get("samples"):
        md += ["", "### CIDR 示例", ""] + [f"- `{item['rule']}` ← `{item['covered_by']['category']}/{item['covered_by']['item']}`" for item in cidr_audit["samples"][:50]]
    if conflicts:
        md += ["", "## 跨分类冲突（最多 100 条）", ""] + [f"- `{rule}` → 胜出 `{winners[rule]}`；涉及 {', '.join(category_list)}" for rule, category_list in list(sorted(conflicts.items()))[:100]]
    md += ["", "## 编译输出校验", "", f"- 状态：**{'PASS' if output_audit['valid'] else 'FAIL'}**", f"- 问题数：`{output_audit['problem_count']}`"]
    (REPORTS / "latest.md").write_text("\n".join(md) + "\n", encoding="utf-8")

    print(json.dumps({"ok": not blocked, "gate": "BLOCKED" if blocked else "PASS", "severity": dict(severity_counts), "conflicts": len(conflicts), "domain_redundancy": domain_audit.get("count", 0), "cidr_redundancy": cidr_audit.get("count", 0), "compiled": compiled}, ensure_ascii=False))
    if blocked:
        return 1
    return 0
