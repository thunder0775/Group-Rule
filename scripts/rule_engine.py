...
def main():
    ...
    for category,items in categories.items():
        merged=list(dict.fromkeys(r for rules in items.values() for r in rules)); write_list(RULES/"compiled"/f"{category}.list",merged,f"compiled:{category}"); compiled[category]=len(merged)
    occ=defaultdict(set)
    for category,items in categories.items():
