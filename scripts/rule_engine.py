...
def valid(text,rules,min_rules=1,previous_source_rules=0,min_rule_ratio=0.2):
    low=text.lower()
    if "<html" in low or "<!doctype html" in low: return "html_instead_of_rules"
    if len(rules) < min_rules: return "too_few_rules"
    if previous_source_rules >= 10 and len(rules) < int(previous_source_rules * min_rule_ratio):
        return "sudden_rule_count_drop"
    return None
...
def main():
    ...
    health=scfg.get("health",{}); min_rules_default=int(health.get("min_rules",1)); min_ratio=float(health.get("min_rule_ratio",0.2))
    ...
    for category,items in scfg["sources"].items():
        for name,sources in items.items():
            ...
            for src in sources:
                ...
                min_rules=int(src.get("min_rules",min_rules_default)); ratio=float(src.get("min_rule_ratio",min_ratio))
                previous_source_rules=source_history.get(url,0)
                try:
                    text,meta=fetch(url,timeout,max_bytes); parsed=parse(text,mode,policy); reason=valid(text,parsed,min_rules,previous_source_rules,ratio)
                    ok=reason is None; item={"url":url,"mode":mode,"policy_filter":policy,"min_rules":min_rules,"previous_source_rules":previous_source_rules,**meta,"rules":len(parsed),"status":"ok" if ok else "rejected"}
    ...
    for category in order + [c for c in categories if c not in order]:
        ...
        for name,rules in categories[category].items():
            for r in rules:
    ...
    for category,items in categories.items():
        ...
        kept=list(dict.fromkeys(kept)); compiled_rules[category]=kept
        write_list(RULES/"compiled"/f"{category}.list",kept,f"compiled:{category}"); compiled[category]=len(kept)
    occ=defaultdict(set)
    for category,items in categories.items():
    ...
    report={"generated_at":now(),"status":statuses,"compiled_counts":compiled,"cross_category_conflicts":conflicts,"conflict_resolution":resolved,"priority":pcfg}
