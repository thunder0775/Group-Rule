...
10. **发布闸门**：`BLOCK` 问题会使 Actions 失败并禁止提交生成结果；`WARNING` 保留 Last Known Good 并继续其他原子规则；`INFO` 仅记录。
11. **历史变化追踪**：记录上游 SHA、规则数、首次发现、最近变化与连续未变化次数。
12. **小火箭只负责分流**：代理策略完全写在自己的 Shadowrocket 配置里。

## 审计层级

`上游可用性 → 内容完整性 → 规则结构 → 规则语义 → 跨分类冲突 → 编译输出 → 发布闸门`

审计结果分为：

- `BLOCK`：禁止本次生成结果进入 Git 提交，例如配置错误、无 Last Known Good 的原子规则不可用、编译输出非法。
- `ERROR`：严重运行异常，作为后续扩展保留级别。
- `WARNING`：源异常、重定向主机变化、危险关键词等，记录并按故障隔离策略继续。
- `INFO`：语义冗余等信息，不改变规则。
...
GitHub Actions 每天自动更新，也支持手动 `workflow_dispatch`。配置、脚本或规则源变更时会自动运行；生成内容无变化则不提交，并通过并发控制避免多个构建同时推送。发布闸门失败时仍会输出完整审计报告，但不会提交生成规则。
